...
def main(fn, model_FT, device, output_dir='./pred_res', station_file='channel_HAGI', pth=0.5, sth=0.5):
    """
...
def run_phase_pick(
    input_dir='./trg_20250401_HAGI',
    output_dir='./pred_res',
    station_file='channel_HAGI',
    model_path='SegPhase/best_model.pth',
    device='cuda:2',
    p_threshold=0.5,
    s_threshold=0.5,
):
    """
    連続 WIN 波形に SegPhase を適用し、REAL 用の P/S ピックファイルを作成する。

    Parameters
    ----------
    input_dir : str
        1分単位の WIN 波形ファイルを保存した入力ディレクトリ。
    output_dir : str
        CT.{station}.P.txt / CT.{station}.S.txt を出力するディレクトリ。
    station_file : str
        WIN ファイル読み込みに使用する観測点リスト。
    model_path : str
        SegPhase の重みファイル。
    device : str
        推論に使うデバイス。例: 'cuda:0', 'cuda:2', 'cpu'。
    p_threshold, s_threshold : float
        P 相・S 相ピーク検出の閾値。
    """
    ...
    total = len(files)
    print(f'[phase_pick] input files: {total}')

    for c, fn in enumerate(files[:-1], start=1):
        print(f'[phase_pick] {c}/{total} - {os.path.basename(fn)}', flush=True)
        try:
            main(fn, model, device, output_dir=output_dir, station_file=station_file, pth=p_threshold, sth=s_threshold)
        except KeyboardInterrupt:
            print('\n[phase_pick] Ctrl+C を受け取りました。処理を中断します。')
            break
        except Exception as e:
            print(f'[phase_pick] error: {fn}: {e}')
            with open(os.path.join(output_dir, 'miss_phase_pick.list'), 'a') as f:
                f.write(f'{fn}\n')
...
def build_argparser():
    ...
    parser = argparse.ArgumentParser(
        description='SegPhase による連続波形の P/S ピック作成スクリプト。'
    )
    parser.add_argument('--input-dir', default='./trg_20250401_HAGI', help='1分単位 WIN 波形ファイルの入力ディレクトリ')
    parser.add_argument('--output-dir', default='./pred_res', help='P/S ピックの出力ディレクトリ')
    parser.add_argument('--station-file', default='channel_HAGI', help='観測点リストファイル')
    parser.add_argument('--model-path', default='SegPhase/best_model.pth', help='SegPhase 重みファイル')
    parser.add_argument('--device', default='cuda:2', help='推論デバイス')
    parser.add_argument('--p-threshold', type=float, default=0.5, help='P 相ピーク検出閾値')
    parser.add_argument('--s-threshold', type=float, default=0.5, help='S 相ピーク検出閾値')
    return parser
