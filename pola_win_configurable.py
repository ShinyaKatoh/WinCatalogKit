...
def load_povit_model(model_path='./PoViT/model_100Hz.pth', device='cuda:2'):
    """
    PoViT の P 波初動極性推定モデルを読み込む。
    ...
    model_path : str
        PoViT の重みファイル。
    device : str
        推論に使うデバイス。例: 'cuda:0', 'cuda:2', 'cpu'。
    """
    device_obj = torch.device(device if torch.cuda.is_available() and str(device).startswith('cuda') else 'cpu')
    model = Model(in_length=256, kernel_size=16, ds_kernel_size=9, ff_kernel_size=9, seg_kernel_size=9, stride=1, head_num=4, emb_dim=64, num_blocks=7, dropout_ratio=0.3)
...
def main(events_df, picks_df, dire, win_root='./trg_20250401_HAGI', station_file='channel_HAGI', model100_P=None, device=None):

    if model100_P is None or device is None:
        model100_P, device = load_povit_model()
    ...
    for event_id, grp in picks_df.groupby("event_id", sort=True):
        ...
        try:
            ...
            ## Waveform load
            winf_1 = create_winf_path(origin_time, root=win_root)
            chf_1 = station_file
            waveform_info_df_1, waveform_dict_1 = extract_each_station_waveform_metadata(winf_1, chf_1, uniq_stations)
            
            winf_2 = create_winf_path(origin_time + np.timedelta64(1, "m"), root=win_root)
            chf_2 = station_file
...
def build_argparser():
    ...
    parser = argparse.ArgumentParser(
        description='REAL の phase_sel.txt と WIN 波形から PoViT 初動極性を推定し、HYPOMH 用入力ファイルを作成するスクリプト。'
    )
    parser.add_argument('--phase-sel', default='./pred_res/phase_sel.txt', help='REAL が出力した phase_sel.txt')
    parser.add_argument('--win-root', default='./trg_20250401_HAGI', help='WIN 波形ファイルの入力ディレクトリ')
    parser.add_argument('--output-root', default=None, help='res_pre_pick/res_pick を作成する出力ディレクトリ。未指定なら phase_sel.txt の親ディレクトリ')
    parser.add_argument('--station-file', default='channel_HAGI', help='WIN ファイル読み込み用観測点リスト')
    parser.add_argument('--model-path', default='./PoViT/model_100Hz.pth', help='PoViT 重みファイル')
    parser.add_argument('--device', default='cuda:2', help='推論デバイス')
    return parser


if __name__ == '__main__':
    args = build_argparser().parse_args()
    events_df, picks_df = read_event_pick_file(args.phase_sel)
    print(events_df.head())
    print(picks_df.head())

    output_root = args.output_root if args.output_root is not None else os.path.dirname(args.phase_sel)
    os.makedirs(output_root, exist_ok=True)

    model100_P, device = load_povit_model(args.model_path, args.device)
    main(
        events_df,
        picks_df,
        output_root,
        win_root=args.win_root,
